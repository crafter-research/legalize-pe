...
const data = JSON.parse(readFileSync(join(__dirname, '../normas-legales-gob.json'), 'utf-8'))
...
function parseNorma(item) {
  ...
  if (url.includes('decreto-legislativo') || titulo.match(/Decreto Legislativo/i)) {
    tipo = 'decreto-legislativo'
    const m = url.match(/decreto[_-]legislativo[_-]?(\d+)/i) || item.detailUrl.match(/\/(\d+)-(\d+)$/)
    if (m) numero = m[2] || m[1]
...
const unique = normas.filter((n, i, arr) => arr.findIndex(x => x.identificador === n.identificador) === i)

const missing = unique.filter(n => !existsSync(join(LEYES_DIR, n.identificador + '.md')))
const existing = unique.filter(n => existsSync(join(LEYES_DIR, n.identificador + '.md')))
...
missing.forEach(n => console.log(' -', n.identificador, '|', n.titulo.slice(0, 80)))
