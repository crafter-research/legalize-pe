...
function isBodyJustFrontmatterRepeat(
...
async function auditLaws(): Promise<AuditReport> {
  ...
  for (const file of files) {
    ...
    if (contentLines < 20) {
      report.emptyOrNearEmpty.push({
        file,
        issues: [`${contentLines} content lines`],
      })
    ...
    const missingFields = REQUIRED_FIELDS.filter(
      (f) => !frontmatter[f] || frontmatter[f].trim() === '',
    )
...
function printReport(report: AuditReport): void {
  ...
  console.log(
    `1. Missing frontmatter:         ${report.summary.missingFrontmatter}`,
  )
  console.log(
    `2. Empty/near-empty (<20 lines): ${report.summary.emptyOrNearEmpty}`,
  )
  console.log(`3. SPIJ artifacts:              ${report.summary.spijArtifacts}`)
  console.log(
    `4. Missing required fields:     ${report.summary.missingRequiredFields}`,
  )
  console.log(
    `5. Placeholder/error content:   ${report.summary.placeholderOrError}`,
  )
  console.log(
    `6. Duplicate identifiers:       ${report.summary.duplicateIdentifiers} groups`,
  )
  console.log(`7. Non-vigente estado:          ${report.summary.nonVigente}`)
  console.log(
    `8. Body lacks legal content:    ${report.summary.bodyOnlyFrontmatter}`,
  )
  console.log('')
